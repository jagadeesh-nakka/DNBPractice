...
const Home = () => {
  ...
  // Sort skills: selected skills come first
  const sortedSkills = [...primarySkills]
    .sort((a, b) => {
      const aSelected = userSkills.find(s => s.id === a.id) ? 1 : 0;
      const bSelected = userSkills.find(s => s.id === b.id) ? 1 : 0;
  ...
  if (selectedSkill) {
    return (
      <SkillForm 
        ...
        onSubmit={(data) => {
          // Save user skills
          handleAddUserSkill(selectedSkill);
          setSelectedSkill(null);
        }}
      />
    );
  ...
  return (
    <div className="home-container">
      ...
      <SkillMatrix 
        ...
        selectedSkills={userSkills.map(s => s.id)} // pass selected IDs for highlighting
      />
    </div>
  );
};
